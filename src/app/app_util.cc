...
AppUtil::StopCallback stop_callback;
AppUtil::IsStoppedCallback is_stopped_callback;
...
extern "C" void SignalHandler(int sig) {
  if (stop_callback && (!is_stopped_callback || !is_stopped_callback())) {
    LOG(INFO) << "Bye Bye";
    stop_callback();
  }
}
...
CLIOptions ParseCommandLineOptions(int argc, char **argv) {
  ...
  if (argc > 0 && argv[0] != nullptr) {
    opts.service_type = DetectServiceType(argv[0]);
  }
...
void PrintVersion(std::ostream &os) { os << "kvrocks version " << VERSION << " @" << GIT_COMMIT << std::endl; }
...
void RegisterStopHandler(IsStoppedCallback is_stopped, StopCallback stop) {
  is_stopped_callback = std::move(is_stopped);
  stop_callback = std::move(stop);
}

void ClearStopHandler() {
  is_stopped_callback = nullptr;
...
void RemovePidFile(const std::string &path) { std::remove(path.data()); }
