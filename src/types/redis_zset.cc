...
std::string ZSetRangeByScoreCacheKey(const std::string &ns_key, uint64_t epoch, const ZRangeSpec &spec) {
  std::string cache_key;
  cache_key.reserve(2 + ns_key.size() + 8 * 3 + 2);
...
// Cache payload starts with the source key's absolute expire timestamp so
// cached range results can self-expire without reading metadata again.
std::string EncodeMemberScores(const std::vector<MemberScore> &mscores, uint32_t expire) {
  std::string encoded;
...
bool DecodeMemberScores(const std::string &encoded, std::vector<MemberScore> *mscores, uint32_t *expire = nullptr) {
  mscores->clear();

  rocksdb::Slice input(encoded);
  uint32_t cached_expire = 0;
  if (!GetFixed32(&input, &cached_expire)) {
    return false;
  }
  if (expire != nullptr) {
    *expire = cached_expire;
  }
  if (cached_expire > 0 && static_cast<uint32_t>(Util::GetTimeStamp()) >= cached_expire) {
    return false;
  }
  ...
  for (uint32_t i = 0; i < size; ++i) {
    ...
    if (!GetFixed32(&input, &member_size) || input.size() < member_size + sizeof(uint64_t)) {
      mscores->clear();
      return false;
    }

    std::string member(input.data(), member_size);
    input.remove_prefix(member_size);

    double score = 0;
    if (!GetDouble(&input, &score)) {
      mscores->clear();
      return false;
    }
    mscores->push_back(MemberScore{std::move(member), score});
  }
...
rocksdb::Status ZSet::Add(const Slice &user_key, ZAddFlags flags, std::vector<MemberScore> *mscores, int *ret) {
  *ret = 0;
  ...
  if (added > 0 || changed > 0) {
    BumpZSetCacheEpoch(storage_, ns_key);
  }
...
rocksdb::Status ZSet::RangeByScore(const Slice &user_key, ZRangeSpec spec, std::vector<MemberScore> *mscores,
  ...
  LockGuard guard(storage_->GetLockManager(), ns_key);
  const bool use_range_cache = !spec.removed && mscores != nullptr;
  std::string range_cache_key;
  uint64_t range_cache_epoch = 0;
  if (use_range_cache) {
    range_cache_epoch = GetZSetCacheEpoch(storage_, ns_key);
    range_cache_key = ZSetRangeByScoreCacheKey(ns_key, range_cache_epoch, spec);
    std::string encoded_mscores;
    if (storage_->Cache().TryGet(range_cache_key, &encoded_mscores)) {
      if (DecodeMemberScores(encoded_mscores, mscores)) {
        if (size) *size = static_cast<int>(mscores->size());
        return rocksdb::Status::OK();
      }
      storage_->Cache().Remove(range_cache_key);
    }
  }
  ...
  if (use_range_cache) {
    storage_->Cache().Put(range_cache_key, EncodeMemberScores(*mscores, metadata.expire));
  }
  return rocksdb::Status::OK();
}
