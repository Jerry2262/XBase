...
enum ServiceType { kServiceDatanode = 0, kServiceProxy, kServiceAll };
...
struct CLIOptions {
  ...
  ServiceType service_type = kServiceAll;
  std::vector<std::pair<std::string, std::string>> cli_options;

  CLIOptions() = default;
  explicit CLIOptions(std::string_view file) : conf_file(file) {}
  CLIOptions(std::string_view file, ServiceType mode) : conf_file(file), service_type(mode) {}
};
...
struct Config {
 public:
  ...
  int service_type = kServiceAll;
  int datanode_port = kDefaultPort + 1;
  int max_db_size = 0;
  ...
  std::string storage_backend_addrs;
  std::string storage_rpc_connection_type;
 ...
 public:
  ...
  bool IsProxy() const { return service_type == kServiceProxy; }
  bool IsDatanode() const { return service_type == kServiceDatanode; }
  bool IsAllInOne() const { return service_type == kServiceAll; }
  bool RunsProxy() const { return IsProxy() || IsAllInOne(); }
  bool RunsDatanode() const { return IsDatanode() || IsAllInOne(); }
  int GetDatanodeListenPort() const { return IsAllInOne() ? datanode_port : port; }
  const char *GetServiceTypeName() const;
