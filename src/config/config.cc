...
Config::Config() {
  ...
  FieldWrapper fields[] = {
      ...
      {"datanode-port", true, new IntField(&datanode_port, kDefaultPort + 1, 1, PORT_LIMIT)},
      ...
      {"storage-rpc-connection-type", true, new StringField(&storage_rpc_connection_type, "single")},
      ...
      {"core-threads", true, new IntField(&core_threads, 0, 0, 256)},
      {"core-queue-size", true, new IntField(&core_queue_size, 10240, 1, INT_MAX)},
      ...
      {"cache.capacity", true, new Int64Field(&KVrocksCache.capacity, 1*1024*1024*1024, 0, INT_MAX)},
      {"cache.count", true, new Int64Field(&KVrocksCache.count, 0, 0, INT_MAX)},
      /* rocksdb options */
...
Status Config::finish() {
  ...
  if (IsAllInOne() && port == datanode_port) {
    return Status(Status::NotOK, "all mode requires port and datanode-port to be different");
  }
  if (proxy_brpc_num_threads > 0 && proxy_brpc_num_threads < 4) {
    return Status(Status::NotOK, "proxy-brpc-num-threads must be 0 or >= 4");
  }
  if (datanode_brpc_num_threads > 0 && datanode_brpc_num_threads < 4) {
    return Status(Status::NotOK, "datanode-brpc-num-threads must be 0 or >= 4");
  }
  if (IsAllInOne() && proxy_brpc_num_threads != datanode_brpc_num_threads) {
    return Status(Status::NotOK,
                  "all mode requires proxy-brpc-num-threads and datanode-brpc-num-threads to be equal");
  }
  if (IsProxy() && storage_backend_addrs.empty()) {
    return Status(Status::NotOK, "proxy mode requires storage-backend-addrs");
  }
  if (IsAllInOne() && storage_backend_addrs.empty()) {
    storage_backend_addrs = "127.0.0.1:" + std::to_string(datanode_port);
  }
...
const char *Config::GetServiceTypeName() const {
  switch (service_type) {
    case kServiceProxy:
      return "proxy";
    case kServiceDatanode:
      return "datanode";
    case kServiceAll:
      return "all";
  }
...
Status Config::Load(const CLIOptions &opts) {
  ...
  if (!opts.conf_file.empty()) {
    ...
    std::cout << "Warn: no config file specified, using the default config. "
                 "In order to specify a config file use kvrocks-all -c /path/to/kvrocks.conf"
              << std::endl;
  }
