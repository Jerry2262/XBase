...
void RemoveHashCacheEntries(Engine::Storage *storage, rocksdb::DB *db, const std::string &ns_key, uint64_t version) {
  std::string prefix_key, next_version_prefix_key;
  InternalKey(ns_key, "", version, storage->IsSlotIdEncoded()).Encode(&prefix_key);
  InternalKey(ns_key, "", version + 1, storage->IsSlotIdEncoded()).Encode(&next_version_prefix_key);
  ...
  auto iter = DBUtil::UniqueIterator(db, read_options);
  for (iter->Seek(prefix_key); iter->Valid() && iter->key().starts_with(prefix_key); iter->Next()) {
    ...
    storage->Cache().Remove(HashCacheKey(ns_key, ikey.GetSubKey()));
  }
}
...
rocksdb::Status Database::Expire(const Slice &user_key, int timestamp) {
  ...
  if (s.ok()) {
    if (metadata.Type() == kRedisString) {
      ...
      RemoveHashCacheEntries(storage_, db_, ns_key, metadata.version);
      storage_->Cache().Remove(HashAllCacheKey(ns_key));
    } else if (metadata.Type() == kRedisZSet) {
