...
inline std::string StringCacheKey(const std::string &ns_key) {
  std::string cache_key;
  cache_key.reserve(1 + ns_key.size());
  cache_key.push_back('s');
  cache_key.append(ns_key);
  return cache_key;
}

inline std::string HashCacheKey(const std::string &ns_key, const Slice &field) {
  std::string cache_key;
  cache_key.reserve(1 + 4 + ns_key.size() + field.size());
  cache_key.push_back('h');
  PutFixed32(&cache_key, static_cast<uint32_t>(ns_key.size()));
  cache_key.append(ns_key);
  cache_key.append(field.data(), field.size());
  return cache_key;
}

inline std::string HashAllCacheKey(const std::string &ns_key) {
  std::string cache_key;
  cache_key.reserve(2 + 4 + ns_key.size());
  cache_key.push_back('h');
  cache_key.push_back('a');
  PutFixed32(&cache_key, static_cast<uint32_t>(ns_key.size()));
  cache_key.append(ns_key);
  return cache_key;
}

inline std::string ZSetEpochCacheKey(const std::string &ns_key) {
  std::string cache_key;
  cache_key.reserve(2 + ns_key.size());
  cache_key.push_back('z');
  cache_key.push_back('e');
  cache_key.append(ns_key);
  return cache_key;
}

inline uint64_t GetZSetCacheEpoch(Engine::Storage *storage, const std::string &ns_key) {
  std::string epoch_value;
  if (!storage->Cache().TryGet(ZSetEpochCacheKey(ns_key), &epoch_value) || epoch_value.size() != sizeof(uint64_t)) {
    return 0;
  }
  return DecodeFixed64(epoch_value.data());
}

inline void SetZSetCacheEpoch(Engine::Storage *storage, const std::string &ns_key, uint64_t epoch) {
  std::string epoch_value;
  PutFixed64(&epoch_value, epoch);
  storage->Cache().Put(ZSetEpochCacheKey(ns_key), epoch_value);
}

inline void BumpZSetCacheEpoch(Engine::Storage *storage, const std::string &ns_key) {
  SetZSetCacheEpoch(storage, ns_key, GetZSetCacheEpoch(storage, ns_key) + 1);
}
