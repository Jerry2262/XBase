...
Status MakeProxyDispatchParseError(const std::string &msg) { return {Status::RedisParseErr, msg}; }

Status EncodeSingleResultReply(const Dispatcher::RequestResult &result, bool treat_as_bulk_string, std::string *reply) {
  if (reply == nullptr) {
    return {Status::RedisExecErr, "reply output is required"};
  }
  if (treat_as_bulk_string) {
    *reply = result.found ? Redis::BulkString(result.value) : Redis::NilString();
  } else if (!result.status.empty()) {
    *reply = Redis::SimpleString(result.status);
  } else {
    *reply = Redis::Integer(result.integer);
  }
  return Status::OK();
}

Status EncodeMultiResultReply(const Dispatcher::MultiRequestResult &result, std::string *reply) {
  if (reply == nullptr) {
    return {Status::RedisExecErr, "reply output is required"};
  }
  ...
  statuses.reserve(result.founds.size());
  for (bool found : result.founds) {
    statuses.emplace_back(found ? rocksdb::Status::OK() : rocksdb::Status::NotFound());
  }
  *reply = Redis::MultiBulkString(result.values, statuses);
  return Status::OK();
}
...
Status Server::Start() {
#ifdef BRPC_FOUND
  request_dispatcher_ = std::make_unique<Dispatcher::RequestDispatcher>(*config_);
  if (request_dispatcher_->init_result() != 0) {
    LOG(ERROR) << "Failed to initialize request dispatcher";
    return Status(Status::NotOK, "failed to initialize request dispatcher");
  }
#endif

  if (storage_ && !config_->master_host.empty()) {
  ...
  } else if (!config_->master_host.empty()) {
    LOG(WARNING) << "Skip replication initialization because local storage is disabled";
  }

  if (config_->cluster_enabled && storage_) {
  ...
  } else if (config_->cluster_enabled) {
    LOG(INFO) << "Skip slot migration initialization because local storage is disabled";
  }
...
Status Server::LocalStorageUnavailable(const std::string &operation) const {
...
Status Server::DispatchProxyCommand(const Redis::CommandAttributes &attributes,
  ...
  if (command_name == "get") {
    auto result = request_dispatcher_->DispatchRequest(Dispatcher::StringGetRequest(ns, cmd_tokens[1], command_flags));
  ...
  if (command_name == "mget") {
    ...
    auto result = request_dispatcher_->DispatchRequest(request);
    if (!result.IsOK()) return result.ToStatus();
  ...
  if (command_name == "del") {
    ...
    auto result = request_dispatcher_->DispatchIntegerCommand("DEL", ns, args, key_arg_indexes, command_flags);
    if (!result.IsOK()) return result.ToStatus();
    return EncodeSingleResultReply(*result, false, reply);
  }
  ...
  if (command_name == "hmget") {
    std::vector<std::string> args(cmd_tokens.begin() + 1, cmd_tokens.end());
    auto result = request_dispatcher_->DispatchArrayCommand("HMGET", ns, args, {0}, command_flags);
    if (!result.IsOK()) return result.ToStatus();
    return EncodeMultiResultReply(*result, reply);
  }
  ...
  if (command_name == "hmset") {
    std::vector<std::string> args(cmd_tokens.begin() + 1, cmd_tokens.end());
    auto result = request_dispatcher_->DispatchStatusCommand("HMSET", ns, args, {0}, command_flags);
    if (!result.IsOK()) return result.ToStatus();
    return EncodeSingleResultReply(*result, false, reply);
  }
  if (command_name == "hgetall") {
    std::vector<std::string> args(cmd_tokens.begin() + 1, cmd_tokens.end());
    auto result = request_dispatcher_->DispatchArrayCommand("HGETALL", ns, args, {0}, command_flags);
    if (!result.IsOK()) return result.ToStatus();
    return EncodeMultiResultReply(*result, reply);
  }
  if (command_name == "lindex") {
    ...
    auto result =
        request_dispatcher_->DispatchRequest(Dispatcher::ListGetRequest(ns, cmd_tokens[1], *index, command_flags));
    if (!result.IsOK()) return result.ToStatus();
    return EncodeSingleResultReply(*result, true, reply);
  }
  ...
  if (command_name == "zadd") {
    ...
    auto result = request_dispatcher_->DispatchSingleCommand("ZADD", ns, args, {0}, command_flags);
    if (!result.IsOK()) return result.ToStatus();
    return EncodeSingleResultReply(*result, zadd_returns_bulk_string, reply);
  }
  if (command_name == "zrangebyscore") {
    std::vector<std::string> args(cmd_tokens.begin() + 1, cmd_tokens.end());
    auto result = request_dispatcher_->DispatchArrayCommand("ZRANGEBYSCORE", ns, args, {0}, command_flags);
    if (!result.IsOK()) return result.ToStatus();
    return EncodeMultiResultReply(*result, reply);
  }
...
Status Server::DispatchProxyCommandAsync(const Redis::CommandAttributes &attributes,
                                         ...
                                         Dispatcher::RequestDispatcher *request_dispatcher) const {
#ifndef BRPC_FOUND
  return {Status::RedisExecErr, "proxy request dispatch requires brpc support"};
#else
  if (HasLocalStorage()) {
  ...
  if (request_dispatcher == nullptr) request_dispatcher = request_dispatcher_.get();
  if (request_dispatcher == nullptr || request_dispatcher->init_result() != 0) {
    return {Status::RedisExecErr, "request dispatcher is not initialized"};
  }
  ...
  if (command_name == "get") {
    return request_dispatcher->DispatchRequestAsync(
  ...
  if (command_name == "del") {
    ...
    std::vector<size_t> key_arg_indexes(args.size());
    std::iota(key_arg_indexes.begin(), key_arg_indexes.end(), 0);
    return request_dispatcher->DispatchIntegerCommandAsync(
  ...
  if (command_name == "hmget") {
    std::vector<std::string> args(cmd_tokens.begin() + 1, cmd_tokens.end());
    return request_dispatcher->DispatchArrayCommandAsync(
  ...
  if (command_name == "hmset") {
    std::vector<std::string> args(cmd_tokens.begin() + 1, cmd_tokens.end());
    return request_dispatcher->DispatchStatusCommandAsync(
  ...
  if (command_name == "zadd") {
    ...
    return request_dispatcher->DispatchSingleCommandAsync(
...
Status Server::AddMaster(std::string host, uint32_t port, bool force_reconnect) {
  if (!HasLocalStorage()) {
    return LocalStorageUnavailable("replication");
  }
...
Status Server::RemoveMaster() {
  ...
  if (!master_host_.empty()) {
    ...
    if (replication_thread_) replication_thread_->Stop();
    replication_thread_ = nullptr;
    if (storage_) storage_->ShiftReplId();
  }
  return Status::OK();
}
