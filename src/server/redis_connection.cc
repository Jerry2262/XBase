...
bool CanExecuteLocallyInProxyMode(const std::string &command_name) {
  return command_name == "auth" || command_name == "command" || command_name == "echo" || command_name == "hello" ||
         command_name == "ping" || command_name == "quit";
}
...
void Connection::ExecuteCommands(std::deque<CommandTokens> *to_process_cmds) {
  ...
  while (!to_process_cmds->empty()) {
    ...
    if (svr_->storage_ == nullptr && !CanExecuteLocallyInProxyMode(cmd_name)) {
      if (IsFlagEnabled(Connection::kMultiExec) || attributes->is_multi()) {
        Reply(Redis::Error("ERR transactions are not supported in proxy mode"));
        multi_error_ = true;
        continue;
      }
