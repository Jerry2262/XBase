...
TEST(Config, CacheKeysAreAccepted) {
  const char *path = "cache_keys.conf";
  unlink(path);

  std::ofstream output_file(path, std::ios::out);
  output_file << "cache.capacity 4096\n";
  output_file << "cache.count 12\n";
  output_file.close();

  Config config;
  ASSERT_TRUE(config.Load(CLIOptions(path)).IsOK());
  EXPECT_EQ(config.KVrocksCache.capacity, 4096);
  EXPECT_EQ(config.KVrocksCache.count, 12);

  std::vector<std::string> values;
  config.Get("cache.capacity", &values);
  ASSERT_EQ(values.size(), 2);
  EXPECT_EQ(values[0], "cache.capacity");
  EXPECT_EQ(values[1], "4096");

  config.Get("cache.count", &values);
  ASSERT_EQ(values.size(), 2);
  EXPECT_EQ(values[0], "cache.count");
...
TEST(AppUtil, DetectServiceTypeFromProgramName) {
  EXPECT_EQ(AppUtil::DetectServiceType("kvrocks-proxy"), kServiceProxy);
  EXPECT_EQ(AppUtil::DetectServiceType("/usr/local/bin/kvrocks-datanode"), kServiceDatanode);
  EXPECT_EQ(AppUtil::DetectServiceType("kvrocks-all"), kServiceAll);
  EXPECT_EQ(AppUtil::DetectServiceType("kvrocks-brpc"), kServiceAll);
}

TEST(Config, ServiceTypeProxyUsesBackend) {
  const char *path = "service_type.conf";
  unlink(path);

  std::ofstream output_file(path, std::ios::out);
  output_file << "storage-backend-addrs 127.0.0.1:16666\n";
  output_file.close();

  Config config;
  auto s = config.Load(CLIOptions(path, kServiceProxy));
  ASSERT_TRUE(s.IsOK());
  EXPECT_TRUE(config.IsProxy());
  EXPECT_STREQ(config.GetServiceTypeName(), "proxy");

  unlink(path);
}

TEST(Config, ServiceTypeAllAutoConfiguresLocalBackend) {
  const char *path = "service_type_all.conf";
  unlink(path);

  std::ofstream output_file(path, std::ios::out);
  output_file << "port 17777\n";
  output_file << "datanode-port 16666\n";
  output_file.close();

  Config config;
  auto s = config.Load(CLIOptions(path, kServiceAll));
  ASSERT_TRUE(s.IsOK());
  EXPECT_TRUE(config.IsAllInOne());
  EXPECT_EQ(config.storage_backend_addrs, "127.0.0.1:16666");
  EXPECT_EQ(config.GetDatanodeListenPort(), 16666);
  EXPECT_STREQ(config.GetServiceTypeName(), "all");

  unlink(path);
}

TEST(Config, ServiceTypeProxyRequiresBackend) {
  const char *path = "proxy_requires_backend.conf";
  unlink(path);
  ...
  auto s = config.Load(CLIOptions(path, kServiceProxy));
  ASSERT_FALSE(s.IsOK());
  EXPECT_NE(s.Msg().find("proxy mode requires storage-backend-addrs"), std::string::npos);

  unlink(path);
}

TEST(Config, ServiceTypeAllRequiresDistinctPorts) {
  const char *path = "service_type_all_same_port.conf";
  unlink(path);

  std::ofstream output_file(path, std::ios::out);
  output_file << "port 16666\n";
  output_file << "datanode-port 16666\n";
  output_file.close();
  ...
  auto s = config.Load(CLIOptions(path, kServiceAll));
  ASSERT_FALSE(s.IsOK());
  EXPECT_NE(s.Msg().find("all mode requires port and datanode-port to be different"), std::string::npos);
