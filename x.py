...
def build(dir: str, jobs: int, ghproxy: bool, ninja: bool, unittest: bool, compiler: str, cmake_path: str, D: List[str],
    ...
    target = ["kvrocks-proxy", "kvrocks-datanode", "kvrocks-all", "kvrocks2redis"]
    if unittest:
...
def package_fpm(package_type: str, release_version: str, dir: str, jobs: int) -> None:
    ...
    copyfile(str(Path(dir) / 'kvrocks-all'), str(package_dir / 'bin' / 'kvrocks'))
    copyfile(str(Path(dir) / 'kvrocks-proxy'), str(package_dir / 'bin' / 'kvrocks-proxy'))
    copyfile(str(Path(dir) / 'kvrocks-datanode'), str(package_dir / 'bin' / 'kvrocks-datanode'))
    copyfile(str(Path(dir) / 'kvrocks-all'), str(package_dir / 'bin' / 'kvrocks-all'))
...
def test_go(dir: str, cli_path: str, rest: List[str]) -> None:
    ...
    binpath = Path(dir).absolute() / 'kvrocks-all'
    basedir = Path(__file__).parent.absolute() / 'tests' / 'gocase'
